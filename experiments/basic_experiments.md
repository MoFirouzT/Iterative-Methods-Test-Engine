...
- `MethodResult` field access (`iter_logs`, `n_iters`, `stop_reason`,
  `final_state`);
...
**Status:** done. **File:** `exp_stage2.jl`.

Same five methods, visualized as (x₁, x₂) trajectories overlaid on log-spaced
contours of f. The figure that makes the Rosenbrock geometry visible.

**Exercises:** vector-valued extras plumbing — specifically `extras[:x_iter]` —
...
**Validates:**
- every row in the DataFrame carries a non-missing `:x_iter` (explicit assertion
  in the script);
